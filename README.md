...
用户查询首先经规则拦截（NER 识别 + 20+ 科室正则）快速命中科室映射，毫秒级直接路由（simple_rag）。未命中时降级至 LLM 结构化路由（Guided JSON），自动输出 route_path 和征召科室列表，决定走简单检索还是多专家会诊。
...
每个科室专家以独立 **ReAct Agent** 运行推理循环：LLM 推理 → 自主决策调用工具 → 文献检索 / 药物冲突查询 → 结果注入 → 继续推理，最多 5 轮迭代，直到 LLM 输出 stop。
...
查询经画像约束改写后，同时执行 Dense 向量检索（语义匹配）和 BM25 稀疏检索（精准药名匹配），通过手写 RRF 融合（k=60）两路结果，再经 BGE-Cross-Encoder 重排去噪。若 Reranker 最高分低于 0.2，触发 CoT 安全退避。
...
成功回答触发 LLM 提取技能三元组（intent + action + departments），随后在 Skill_Mem 中检索相似技能（cosine ≥ 0.75），按版本差异执行 add / merge / discard 决策，写入时附带 version、usage_count 和 provenance 元数据。
...
环境变量（.env）> config/custom.yaml > config/default.yaml，环境变量拥有最高优先级，YAML 文件按层级覆盖。
...
- **在线监控**（GET /api/metrics）：会话级聚合指标，实时查看成功率、退避率、路由分布、各阶段平均延迟
- **离线评估**（evaluate_rag.py）：MedQA test.jsonl 批量评测，支持检索 MRR/Hit@k 和生成准确率
- **Harness 7 维评分**（GET /api/harness/evaluate）：零 LLM 调用，完全可复现的确定性评分，支持与基线对比检测 regression
...
每条请求生成唯一 `trace_id`，以 Span 树形式贯穿全链路——路由决策 → 反思拦截 → 检索（KB + 画像约束）→ 重排（Reranker）→ 各科室 ReAct（含工具调用子 Span）→ 共识提炼 → DecisionMaker → 反思 + 技能提取。API 端点支持查询最近追踪记录及单条 Span 树详情。
